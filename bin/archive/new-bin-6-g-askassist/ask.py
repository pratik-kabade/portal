...
# curl -X POST -H "Content-Type: application/json" -d '{"question" : "what is the name of rdkb-370?", "project_name" : "RDKB"}' http://localhost:5000/api/ask
...
def ask():
    ...
    response = str(queryengine.chat(res))
...
def check_jira(project_name, prompt):
    ...
    #id_prompt=f'Extract the issue ID, in format RDKB-xxx, from the following prompt after tag ##PROMPT## and return the issue id in capital letters only. If the prompt does not have any issue id or anything related to jira then return "False" ##PROMPT## {prompt}'
