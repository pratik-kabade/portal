...
class LLM:
    ...
    def rag_model(self, file_name, prompt):
        ...
        Settings.llm = Ollama(model=self.llm_model, request_timeout=120.0, temperature=0)
...
if __name__ == "__main__":
    ...
    # print(llm_instance.rag_model('old-data/file_1.pdf', 'what is this document about?'))
    # print(llm_instance.rag_model('old-data/file_1.pdf', 'what is the name of person mentioned here?'))
    # llm_instance.get_response('hi')
    # print(llm_instance.fetch_entire_response('hi'))
    # print(llm_instance.rag_model('sample.txt', 'what is this document about?'))
    # print(llm_instance.rag_model('alphabet/dummy-jira.csv', 'list all issues which have empty Issue observed, Steps to reproduce, Expected result?'))
